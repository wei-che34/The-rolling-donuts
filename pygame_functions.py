...
# Documentation at www.github.com/stevepaget/pg_functions
# Report bugs at https://github.com/StevePaget/pg_Functions/issues
...
def endWait():
    ...
    while waiting:
        for event in pg.event.get():
            if event.type == pg.QUIT or (event.type == pg.KEYDOWN and event.key == keydict["esc"]):
                waiting = False
    pg.quit()
    exit()
...
def tick(fps):
    for event in pg.event.get():
        if (event.type == pg.KEYDOWN and event.key == keydict["esc"]) or event.type == pg.QUIT:
            pg.quit()
            sys.exit()
    gameClock.tick(fps)
...
if __name__ == "__main__":
    print(""""pg_functions is not designed to be run directly.
    See the wiki at https://github.com/StevePaget/pg_Functions/wiki/Getting-Started for more information""")